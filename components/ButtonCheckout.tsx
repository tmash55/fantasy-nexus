...
const ButtonCheckout = ({
  ...
  const handlePayment = async () => {
    ...
    try {
      if (!user) {
        const backTo = typeof window !== 'undefined' ? window.location.href : '/pricing';
        router.push(`/sign-up?redirectTo=${encodeURIComponent(backTo)}`);
        return;
      }
      const successUrl = `${window.location.origin}/nfl/rankings`;
